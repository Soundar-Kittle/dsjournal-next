...
async function fetchJournal(slug) {
  ...
  const url = `${proto}://${host}/api/journals?slug=${encodeURIComponent(
    slug
  )}`;
  ...
  const arr = Array.isArray(data?.journals)
    ? data.journals
    : data?.journals
    ? [data.journals]
    : [];
...
export default async function JournalDetailPage({ params }) {
  ...
  const menu = [{ menu_label: journal?.short_name, menu_link: `/${slug}` }];

  return (
    <main>
      <header>
        <PageHeader size="text-[25px]" title={`${journal.journal_name} (${journal.short_name})`} image={journal?.banner_image} />
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10">
          ...
          <nav className="mb-4 text-sm text-slate-500">
            <ol className="flex items-center gap-2">
              ...
              <li>/</li>
              <li>
                <Link href="/journals" className="hover:text-slate-700">
                  Journals
                </Link>
              </li>
              <li>/</li>
            ...
            <main className="lg:col-span-3 space-y-8">
              ...
              <div className="rounded-md border shadow-sm bg-white p-6 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 text-sm space-y-2">
                  <p>
                    <b>Editor in Chief:</b> Prof. [Fill from editorial_members
                    table]
                  </p>
                  {journal.issn_online && (
                    <p>
              ...
              <section id="aim" className="space-y-3">
                ...
                <p className="text-slate-700">
                  {journal.aim_scope ||
                    "This journal covers research topics in ..."}
                </p>
              </section>
