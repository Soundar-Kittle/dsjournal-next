...
const onSubmit = async (data) => {
  if (!data.title.trim()) return; // prevent empty submission
  ...
  try {
    if (editingId) {
      await axios.patch(`/api/editorial-titles`, { id: editingId, ...formData });
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button className="cursor-pointer"  size="sm" variant="outline" onClick={() => handleEdit(row.original)}><Pencil className="w-4 h-4" /></Button>
          <Button className="cursor-pointer"  size="sm" variant="destructive" onClick={() => handleDelete(row.original.id,row.original.title)}><Trash2 className="w-4 h-4" /></Button>
        </div>
      ),
    }),
  ];
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Switch checked={field.value} className="cursor-pointer" onCheckedChange={field.onChange} />
            )}
          />
        </div>
